...
## ⚖️ Logical Transition: Why Supabase was Removed?

Around **Feb 24, 2026**, Indian internet service providers began blocking access to Supabase’s core infrastructure (`*.supabase.co`) under a government order, reportedly citing powers under **Section 69A of India’s Information Technology Act**.

*   **Connectivity Failure**: This block caused APIs, database access, and authentication calls to timeout for developers and users within India.
*   **Infrastructure Context**: This was not a downtime on the Supabase side—the platform’s servers and global infrastructure remained fully operational—but network/DNS-level restrictions inside India prevented traffic from reaching it.
*   **Strategic Fix**: Migrating to **Cloudflare R2** with white-labeled custom domains (`i.cdn.avrxt.in`, `v.cdn.avrxt.in`) ensures that our assets remain accessible and immune to infrastructure-wide DNS blocks.
