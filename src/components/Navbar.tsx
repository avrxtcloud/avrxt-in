...
export default function Navbar() {
    ...
    return (
        <>
            ...
            {/* Search Modal - Moved Outside */}
            {isSearchOpen && (
                <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[10vh] sm:pt-[15vh] px-4 sm:px-6">
                    <div className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={() => setIsSearchOpen(false)} />
                    <div className="relative w-full max-w-2xl bg-zinc-900 border border-white/10 rounded-3xl sm:rounded-[2rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-4 sm:p-6 flex items-center gap-4 border-b border-white/5 bg-black/20">
                            ...
                            <input
                                ...
                                className="bg-transparent border-none outline-none text-white w-full text-base sm:text-lg placeholder:text-zinc-700 font-mono"
                            />
                            <button onClick={() => setIsSearchOpen(false)} className="text-zinc-600 hover:text-white font-mono text-[10px] uppercase tracking-widest">Esc</button>
                        </div>
                        <div className="max-h-[60vh] overflow-y-auto p-2 sm:p-4 space-y-1 custom-scrollbar">
                            {filteredSearchItems.map((item) => (
                                <Link
                                    ...
                                    className="group flex items-center justify-between p-3 sm:p-4 rounded-xl hover:bg-white/[0.04] border border-transparent hover:border-white/5 transition-all"
                                >
                                    <div>
                        ...
                        <div className="px-6 py-3 bg-black/40 border-t border-white/5 flex justify-between items-center text-[9px] font-mono text-zinc-600 uppercase tracking-widest">
                            <span>{filteredSearchItems.length} routes</span>
            ...
            {/* Mobile Menu - Moved Outside & Optimized */}
            {isOpen && (
                <div className="fixed inset-0 z-[105] bg-[#0a0a0a] flex flex-col pt-32 px-10 animate-in fade-in duration-300 overflow-hidden">
                    {/* Background Texture */}
                    ...
                    <div className="relative z-10 space-y-4">
                        <span className="text-[10px] font-mono text-zinc-700 uppercase tracking-[0.4em] mb-4 block">Navigation_Index</span>
                        {navLinks.map((link, i) => (
                            <Link
                                key={link.href}
                                href={link.href}
                                onClick={() => setIsOpen(false)}
                                className="block text-4xl sm:text-5xl font-black tracking-tighter text-zinc-800 hover:text-white transition-all transform hover:translate-x-2"
                        ...
                        <Link 
                            href="/me" 
                            onClick={() => setIsOpen(false)}
                            className="block text-4xl sm:text-5xl font-black tracking-tighter text-emerald-500/40 hover:text-emerald-400 transition-all hover:translate-x-2"
                        >
                            /me
                        </Link>
                    </div>
                    ...
                    <div className="relative z-10 mt-auto pb-12 sm:pb-20">
                        <Link 
                            href="/contact" 
                            onClick={() => setIsOpen(false)}
                            className="block w-full bg-white text-black text-center py-5 rounded-2xl font-black uppercase tracking-widest text-xs active:scale-95 transition-all shadow-[0_20px_50px_rgba(255,255,255,0.1)]"
                        >
                            Initiate_Contact
                        </Link>
                    </div>
