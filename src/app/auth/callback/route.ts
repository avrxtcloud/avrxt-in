...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    if (!error && data.user && data.session) {
      const isAdminPath = next.startsWith('/me/admin') || next.startsWith('/docs/admin') || next.startsWith('/mail/admin');
      const source = searchParams.get('source');
      const isSourceAdmin = source === 'admin' || source === 'mail';
      ...
      if (isAdminRequest) {
        const isDiscord = data.user.app_metadata?.provider === 'discord' || data.user.app_metadata?.providers?.includes('discord');
        if (!isDiscord) {
          await supabase.auth.signOut();
          return NextResponse.redirect(new URL(`/auth/login?source=${source || 'admin'}&error=discord_required`, request.url));
        }
        ...
        if (!discordId || !providerToken) {
          await supabase.auth.signOut();
          return NextResponse.redirect(new URL(`/auth/login?source=${source || 'admin'}&error=metadata_missing`, request.url));
        }
        ...
        if (!hasAccess) {
          await supabase.auth.signOut();
          return NextResponse.redirect(new URL(`/auth/login?source=${source || 'admin'}&error=unauthorized_role`, request.url));
        }
