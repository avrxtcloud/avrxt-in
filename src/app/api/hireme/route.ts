...
export async function POST(request: NextRequest) {
    try {
        ...
        // --- STEP 1: SAVE TO GOOGLE SHEET (NON-BLOCKING) ---
        ...
        if (privateKey && process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL && process.env.INTAKE_SHEET_ID) {
            try {
                ...
                console.log('HireMe: Google Sheet updated successfully.');
            } catch (sheetError) {
                console.error('HireMe: Google Sheets Error (Non-blocking):', sheetError);
                // We don't throw here so the email notification can still proceed
            }
        }
